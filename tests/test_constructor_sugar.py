...
from zanim import (
    Circle, Color, Group2D, Object2D, Polygon, Scene, Style, Transform2D, Vec2,
    affine2d,
)
...
class ConstructorSugarTests(unittest.TestCase):
    ...
    def test_object_style_sugar_is_explicit(self):
        outline = Object2D(Circle(1), stroke=WHITE, stroke_width=.045)
        self.assertEqual(outline.style, Style.outline(WHITE, .045))

        fill = Object2D(Circle(1), fill=RED.with_alpha(128))
        self.assertEqual(fill.style, Style.solid(RED.with_alpha(128)))

        painted = Object2D(
            Circle(1), fill=RED.with_alpha(128), stroke=WHITE, stroke_width=.05
        )
        self.assertEqual(
            painted.style, Style.paint(RED.with_alpha(128), WHITE, .05)
        )

    def test_style_and_style_sugar_cannot_mix(self):
        with self.assertRaisesRegex(ValueError, "either style="):
            Object2D(Circle(1), style=Style.solid(RED), stroke=WHITE)
        with self.assertRaisesRegex(ValueError, "stroke_width requires"):
            Object2D(Circle(1), fill=RED, stroke_width=.04)

    def test_object_transform_sugar_uses_fixed_affine_order(self):
        obj = Object2D(
            Circle(1), position=(2, -1), rotation=.3, scale=(2, .5), shear=(.1, -.2)
        )
        self.assertEqual(
            obj.transform,
            affine2d(to=(2, -1), rotation=.3, scale=(2, .5), shear=(.1, -.2)),
        )

    def test_transform_and_transform_sugar_cannot_mix(self):
        with self.assertRaisesRegex(ValueError, "either transform="):
            Object2D(Circle(1), transform=Transform2D.translation(1, 0), scale=2)

    def test_group_transform_sugar_is_parent_relative(self):
        child = Object2D(Circle(.2))
        group = Group2D([child], position=(2, 1), rotation=math.pi / 2)
        expected = affine2d(to=(2, 1), rotation=math.pi / 2)
    ...
    def test_bound_style_sugar_is_only_timeline_sugar(self):
        obj = Object2D(Circle(1), stroke=WHITE)
        scene = Scene()
        obj = scene.add(obj)

        obj.paint(fill=RED.with_alpha(128), stroke=RED, stroke_width=.045)
        self.assertEqual(obj.raw.style, Style.paint(RED.with_alpha(128), RED, .045))
        self.assertEqual(len(scene.timeline.clips), 1)

        obj.outline(WHITE, width=.02)
        self.assertEqual(obj.raw.style, Style.outline(WHITE, .02))
