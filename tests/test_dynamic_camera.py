...
from zanim import Camera2D, Easing, Object2D, Scene, Square, Transform2D, Vec2
...
class BoundCameraSugarTests(unittest.TestCase):
    ...
    def test_camera_pose_uses_rigid_interpolation(self):
        ...
        scene = Scene()
        scene.add(Object2D(Square(1)))
        scene.camera.pose(to=(2, 1), rotation=math.pi / 2, duration=2)
    ...
    def test_camera_pan_is_world_camera_motion(self):
        scene = Scene()
        scene.camera.affine(to=(0, 0), scale=2, duration=0)
