...
from zanim import (
    CENTER, TOP, TOP_LEFT, Canvas, Circle, Column, Grid, Group2D, Object2D,
    Rectangle, Row, Scene, Square, Vec2,
)
...
class LayoutTests(unittest.TestCase):
    def test_frame_exposes_visual_points(self):
        ...
        self.assertEqual(scene.frame.inset(.5).center, Vec2())

    def test_place_names_object_anchor_and_world_target(self):
        obj = Object2D(Rectangle(2, 1))
    ...
    def test_row_is_a_pure_layout_spec_until_place(self):
        a = Object2D(Square(1))
        b = Object2D(Rectangle(2, 1))
        layout = Row(gap=.5, at=Vec2(1, 2))
        ...
        layout.place(a, b)
        self.assertAlmostEqual(b.bounds().left - a.bounds().right, .5)
        self.assertAlmostEqual((a.bounds().left + b.bounds().right) * .5, 1.0)

    def test_column_can_align_left_edges(self):
        a = Object2D(Square(1))
        b = Object2D(Rectangle(2, 1))
        Column(gap=.4, align=Vec2(-1, 0), anchor=TOP, at=Vec2(3, 2)).place(a, b)
    ...
    def test_scene_layout_animates_group_children_independently(self):
        items = [Object2D(Circle(.3)) for _ in range(4)]
        Row(gap=.4, at=Vec2(0, 1.5)).place(*items)
        group = Group2D(items)
        scene = Scene()
        scene.add(group)
        scene.layout(group, to=Grid(rows=2, cols=2, gap=.6, at=Vec2()), duration=2)
        self.assertEqual(len(scene.timeline.clips), 4)
        self.assertEqual(scene.timeline.cursor, 2)
    ...
    def test_layout_can_participate_in_outer_parallel_block(self):
        ...
        scene.add(group)
        with scene.parallel():
            clips = scene.layout(group, to=Row(gap=.5), duration=2)
