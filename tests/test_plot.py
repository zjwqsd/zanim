...
from zanim.plot import Axes2D, DynamicGeometryObject2D


class AxesTests(unittest.TestCase):
    def test_coordinate_round_trip(self):
        axes = Axes2D((-4, 4), (-2, 4), width=8, height=6)
        p = axes.c2p(1.25, -0.5)
        c = axes.p2c(p)
    ...
    def test_area_tracks_bounds(self):
        axes = Axes2D((-4, 4), (-1, 4), width=8, height=5)
        f = lambda x: 1.0 + 0.2*x*x
    ...
    def test_dynamic_geometry_is_random_access(self):
        axes = Axes2D((-3, 3), (-1, 3), width=6, height=4)
    ...
    def test_axis_labels_are_regular_group_objects(self):
        axes = Axes2D((-2,2),(-1,2),width=4,height=3)
