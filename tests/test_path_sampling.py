...
from zanim.path import (
    flatten_cubic,
    resample_polyline_by_arclength,
    sample_vector_contour_by_arclength,
)
...
class PathSamplingTests(unittest.TestCase):
    def test_closed_square_is_uniformly_resampled_without_duplicate_endpoint(self):
        contour = VectorContour(
            (
                CubicBezierGeometry(Vec2(0, 0), Vec2(1 / 3, 0), Vec2(2 / 3, 0), Vec2(1, 0)),
                CubicBezierGeometry(Vec2(1, 0), Vec2(1, 1 / 3), Vec2(1, 2 / 3), Vec2(1, 1)),
                CubicBezierGeometry(Vec2(1, 1), Vec2(2 / 3, 1), Vec2(1 / 3, 1), Vec2(0, 1)),
                CubicBezierGeometry(Vec2(0, 1), Vec2(0, 2 / 3), Vec2(0, 1 / 3), Vec2(0, 0)),
            ),
            True,
        )
        ...
        distances = [
            math.hypot(b.x - a.x, b.y - a.y) for a, b in zip(points, (*points[1:], points[0]))
        ]
        self.assertLess(max(distances) - min(distances), 1e-8)

    def test_flatten_curved_cubic_refines(self):
        curve = CubicBezierGeometry(Vec2(0, 0), Vec2(0, 1), Vec2(1, 1), Vec2(1, 0))
