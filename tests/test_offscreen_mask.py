...
class OffscreenMaskTests(unittest.TestCase):
    def test_scene_raster_source_keeps_transparent_background(self):
        ...
        sub.add(Object2D(Circle(1), style=Style(fill=Color(255, 0, 0), stroke=None)))
        sub.wait(1)
    ...
    def test_alpha_mask_multiplies_content_alpha(self):
        ...
        content.add(Object2D(Circle(1.3), style=Style(fill=Color(255, 0, 0), stroke=None)))
        content.wait(1)
        mask = Scene(canvas=canvas, fps=10)
        mask.add(Object2D(Circle(0.5), style=Style(fill=Color(255, 255, 255), stroke=None)))
        mask.wait(1)
