...
from zanim import WORLD, Circle, Dot, Easing, Scene, Vec2
...
def test_circle():
    ...
    pts = [scene._world_transform(dot.raw, time=t).apply(Vec2()) for t in (0, 0.5, 1, 1.5, 2)]
...
def test_snapshot():
    ...
    dot.move_along(circle, duration=2, easing=Easing.LINEAR)
    mid = scene._world_transform(dot.raw, time=3).apply(Vec2())
