...
class BoundAuthoringTests(unittest.TestCase):
    ...
    def test_affine_has_fixed_explicit_composition_order(self):
        ...
        scene = Scene()
        obj = scene.add(raw)
        obj.affine(position=(2, -1), rotation=0.3, scale=(2, 0.5), shear=(0.2, -0.1), duration=0)
        expected = (
            Transform2D.translation(2, -1)
            @ Transform2D.rotation(0.3)
    ...
    def test_pure_pose_and_affine_factories_match_bound_sugar(self):
        ...
        self.assertEqual(
            ...
            Transform2D.translation(2, 1)
            @ Transform2D.rotation(0.4)
            @ Transform2D.shear(0.1, -0.2)
            @ Transform2D.scaling(2, 0.5),
        )
    ...
    def test_scene_relations_accept_bound_handles(self):
        scene = Scene()
        a, b = scene.add(Square(1), Circle(1))
        scene.interpolate(a, b, duration=0.5)
