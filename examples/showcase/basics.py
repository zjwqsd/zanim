...
def build_scene() -> Scene:
    ...
    square = Object2D(
        Square(1.25), fill=BLUE.with_alpha(185), stroke=Color(220, 232, 255), trim=0
    )
    circle = Object2D(
        Circle(0.68), fill=ORANGE.with_alpha(185), stroke=Color(220, 232, 255), trim=0
    )
    ...
    shapes = Group2D([square, circle, dot])
    ...
    shapes = scene.on(shapes)
    square = scene.on(square)
    circle = scene.on(circle)
    dot = scene.on(dot)
    arrow = scene.on(arrow)
    ...
    scene.camera.affine(to=(-0.3, -0.08), scale=1.15, duration=1.3)
