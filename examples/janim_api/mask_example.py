...
from zanim import (
    BatchObject2D, Canvas, Circle, CircleSet, Color, DynamicGeometryObject2D,
    Easing, Group2D, Object2D, Polygon, RasterObject2D, Rectangle, RIGHT,
    Scene, Style, Text, Transform2D, Vec2, WORLD, affine2d,
)
...
def _stage1(duration=4.0) -> Scene:
    ...
    with content.parallel():
        for i,(c,target) in enumerate(zip(chars,finals)):
            content.transform(c, to=target, duration=1.15, easing=Easing.SMOOTHSTEP, at=0.55+i*0.09)
    content.wait(max(0.0,duration-content.timeline.cursor))

    mask=_subscene()
    mask.add(Object2D(Rectangle(6.25,1.28), position=(0,0.45), fill=WHITE))
    mask.wait(duration)
...
def _stage2(duration=9.8) -> Scene:
    ...
    stage=_subscene()
    brown=Object2D(Rectangle(3,3), fill=LIGHT_BROWN, opacity=0, z_index=-2)
...
def _circle_mask(center: Vec2, duration: float) -> SceneRasterSource:
    sc=_subscene(); sc.add(Object2D(Circle(1.5), position=center, fill=WHITE)); sc.wait(duration)
...
def _stage3(duration=6.0) -> Scene:
    ...
    union_scene.add(
        Object2D(Circle(1.5), position=(-1,0), fill=WHITE),
        Object2D(Circle(1.5), position=(1,0), fill=WHITE),
    ...
    stage=_subscene()
    d1=Object2D(Circle(1.5), position=(-1,0), fill=YELLOW.with_alpha(64))
    d2=Object2D(Circle(1.5), position=(1,0), fill=YELLOW.with_alpha(64))
...
def _stage4(duration=8.9) -> Scene:
    ...
    stage=_subscene(); layer=_full(masked); stage.add(layer); stage.media(layer,duration=duration)
    return stage
