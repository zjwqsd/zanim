...
class _EffectSource(RasterSource):
    def __init__(self, source: RasterSource):
        self.source=source; self.width=source.width; self.height=source.height
        self.duration=source.duration; self.frame_count=source.frame_count
...
class GradientSource(_EffectSource):
    def frame_at(self,t:float)->RasterFrame:
        ...
        if t<2: return frame
        image=PILImage.frombuffer('RGBA',(self.width,self.height),frame.rgba,'raw','RGBA',0,1)
        r,g,b,a=image.split()
        vertical=PILImage.linear_gradient('L').resize((self.width,self.height))
        horizontal=vertical.transpose(PILImage.Transpose.ROTATE_90).resize((self.width,self.height))
...
class GlitchSource(_EffectSource):
    def frame_at(self,t:float)->RasterFrame:
        ...
        rs=ImageChops.offset(r,offset); bs=ImageChops.offset(b,-offset)
        a1=ImageChops.offset(a,offset); a2=ImageChops.offset(a,-offset)
        alpha=ImageChops.lighter(a,ImageChops.lighter(a1,a2))
        lines=PILImage.new('L',(self.width,self.height),0); draw=ImageDraw.Draw(lines)
        for y in range(self.height):
            uv=y/max(1,self.height-1)
            if (uv*10.0+t)%1.0>=0.5:
                draw.line((0,y,self.width,y),fill=255)
        rs=ImageChops.multiply(rs,lines); bs=ImageChops.multiply(bs,lines)
        return RasterFrame(self.width,self.height,PILImage.merge('RGBA',(rs,g,bs,alpha)).tobytes())
