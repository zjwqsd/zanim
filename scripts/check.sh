...
from zanim import Canvas, Circle, Color, Cube3D, Image, Scene, Style, Transform2D, Transform3D
...
with tempfile.TemporaryDirectory(prefix='zanim-check-') as tmp:
    ...
    scene.add(
        Circle(0.8, style=Style(fill=Color(100, 180, 255), stroke=None), z_index=0),
        Image(image_path, width=1.5, transform=Transform2D.rotation(0.2), z_index=1),
        Circle(0.18, style=Style(fill=Color(255, 240, 100), stroke=None), z_index=2),
    )
