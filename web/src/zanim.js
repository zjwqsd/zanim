...
class PolylineInterpolation extends ZObject {
...
function setWorldCanvasTransform(renderer, ctx, m) {
...
class CachedBatch2D extends ZObject {
...
class PrimitiveInterpolation extends ZObject {
...
function applyStyle(o,style){if(!style)return;if('fill' in o)o.fill=style.fill;if('stroke' in o)o.stroke=style.stroke;if('width' in o&&style.width!=null)o.width=style.width;if('worldStroke' in o)o.worldStroke=!!style.worldStroke;}
function lerpStyleState(a,b,t){
...
function cloneState(o){return{transform:o.transform,opacity:o.opacity,reveal:'reveal' in o?Number(o.reveal):null,style:snapshotStyle(o)};}
function assignState(o,s){if(s.transform)o.transform=s.transform;if(s.opacity!=null)o.opacity=s.opacity;if(s.reveal!=null&&'reveal' in o)o.reveal=s.reveal;if(s.style!==undefined)applyStyle(o,s.style);}
function appendOrdered(map,key,clip){
...
export class Scene {
  ...
  static async create(canvas,{wasmURL=DEFAULT_WASM_URL,wasm=null,renderer={},fps=60,observeResize=true}={}){
    ...
    const engine=wasm??await ZanimWasm.load(wasmURL),scene=new Scene(new CanvasRenderer(target,engine,renderer),{fps});
    if(observeResize&&typeof ResizeObserver!=='undefined'){scene._resizeObserver=new ResizeObserver(()=>scene.render());scene._resizeObserver.observe(target);}
    scene.render();return scene;
  }
  _track(object,birth=this.cursor){if(!this._trackedObjects.has(object.id)){this._trackedObjects.set(object.id,object);object._scene=this;object.birth=birth;this.initial.set(object.id,cloneState(object));if(object instanceof CachedBatch2D)this._batchInitial.set(object.id,object.items.map(item=>[...item]));if(object instanceof Group)for(const child of object.children){child._parent=object;this._track(child,birth);}}return object;}
  add(...objects){for(const o of objects){if(!this.objects.includes(o)){this.objects.push(o);this._renderListDirty=true;}this._track(o,this.cursor);}return objects.length===1?objects[0]:objects;}
  addLater(...objects){return this.add(...objects);}
  remove(...objects){for(const o of objects)o.death=this.cursor;return this;}
  invalidateOrder(){this._renderListDirty=true;return this;}
  addValue(...values){for(const v of values)if(!this.values.includes(v))this.values.push(v);return values.length===1?values[0]:values;}
  animateValue(value,{to,duration=1,easing=Easing.SMOOTHSTEP,at=this.cursor}={}){if(!this.values.includes(value))this.addValue(value);const before=this.valueAt(value,at),clip={value,start:at,end:at+duration,before,after:Number(to),easing};this.valueClips.push(clip);appendOrdered(this._valueClipsByValue,value.id,clip);this.cursor=Math.max(this.cursor,clip.end);this.duration=Math.max(this.duration,clip.end);value.value=Number(to);return value;}
  valueAt(value,time){let out=value.initial;const list=this._valueClipsByValue.get(value.id)??[];for(const clip of list){if(time<clip.start)break;if(time>=clip.end){out=clip.after;continue;}const t=clip.easing((time-clip.start)/(clip.end-clip.start));out=lerpNumber(clip.before,clip.after,t);break;}return out;}
  wait(seconds=1){this.cursor+=seconds;this.duration=Math.max(this.duration,this.cursor);return this;}
  at(seconds){this.cursor=seconds;this.duration=Math.max(this.duration,this.cursor);return this;}
  _scheduleClip(object,clip){this.clips.push(clip);appendOrdered(this._clipsByObject,object.id,clip);this.cursor=Math.max(this.cursor,clip.end);this.duration=Math.max(this.duration,clip.end);return clip;}
  animate(object,{transform=undefined,opacity=undefined,reveal=undefined,style=undefined,duration=1,easing=Easing.SMOOTHSTEP,at=this.cursor}={}){
    if(!this.initial.has(object.id))this._track(object,at);const before=this.stateAt(object,at),changes={};
    ...
    const clip={kind:'state',object,start:at,end:at+duration,easing,changes};this._scheduleClip(object,clip);
    const authored={...before};for(const [key,v] of Object.entries(changes))authored[key]=v.after;assignState(object,authored);return object;
  }
  transformFunction(object,provider,{duration=1,easing=Easing.SMOOTHSTEP,at=this.cursor}={}){if(!this.initial.has(object.id))this._track(object,at);const before=this.stateAt(object,at),target=provider(1);if(!(target instanceof Transform2D))throw new TypeError('transformFunction provider must return Transform2D');const clip={kind:'transformFunction',object,start:at,end:at+duration,easing,provider,before:before.transform,after:target};this._scheduleClip(object,clip);object.transform=target;return object;}
  fadeIn(object,{duration=1,easing=Easing.SMOOTHSTEP,at=this.cursor}={}){const before=this.stateAt(object,at);if(Math.abs(before.opacity)>1e-12)throw new Error(`fadeIn() requires opacity 0, got ${before.opacity}`);return this.animate(object,{opacity:1,duration,easing,at});}
  fadeOut(object,{duration=1,easing=Easing.SMOOTHSTEP,at=this.cursor}={}){return this.animate(object,{opacity:0,duration,easing,at});}
  style(object,{to,duration=1,easing=Easing.SMOOTHSTEP,at=this.cursor}={}){if(!snapshotStyle(object))throw new TypeError('style() requires a styled 2D object');return this.animate(object,{style:to,duration,easing,at});}
  trim(object,{to,duration=1,easing=Easing.SMOOTHSTEP,at=this.cursor}={}){if(!('reveal' in object))throw new TypeError('trim() requires a path-trimmable object');if(!(to>=0&&to<=1))throw new RangeError('trim target must be in [0,1]');return this.animate(object,{reveal:to,duration,easing,at});}
  ...
  parallel(durationOrCallback,maybeCallback){
    ...
    const base=this.cursor;let maxEnd=base;const wrap=(method,defaultDuration=1)=>(...args)=>{let opts=args.at(-1);if(!opts||typeof opts!=='object'||Array.isArray(opts)||opts instanceof ZObject||opts instanceof ScalarValue){opts={};args.push(opts);}const duration=opts.duration??shared??defaultDuration,at=base+(opts.at??0);args[args.length-1]={...opts,duration,at};const result=method(...args);maxEnd=Math.max(maxEnd,at+duration);return result;};
    const api={
      animate:wrap((obj,opts)=>this.animate(obj,opts)),animateValue:wrap((v,opts)=>this.animateValue(v,opts)),transformFunction:wrap((obj,p,opts)=>this.transformFunction(obj,p,opts)),
      fadeIn:wrap((obj,opts)=>this.fadeIn(obj,opts)),fadeOut:wrap((obj,opts)=>this.fadeOut(obj,opts)),create:wrap((obj,opts)=>this.create(obj,opts)),style:wrap((obj,opts)=>this.style(obj,opts)),batch:wrap((obj,opts)=>this.batch(obj,opts)),
      move:wrap((obj,by,opts)=>this.move(obj,by,opts)),rotate:wrap((obj,by,opts)=>this.rotate(obj,by,opts)),scale:wrap((obj,by,opts)=>this.scale(obj,by,opts)),affine:wrap((obj,opts)=>this.affine(obj,opts)),
      interpolate:(source,target,opts={})=>{const duration=opts.duration??shared??1,at=base+(opts.at??0),v=this.interpolate(source,target,{...opts,duration,at});maxEnd=Math.max(maxEnd,at+duration);return v;}
    };
    callback(api);this.cursor=maxEnd;this.duration=Math.max(this.duration,maxEnd);return this;
  }
  stateAt(object,time){let state={...(this.initial.get(object.id)??cloneState(object))};const list=this._clipsByObject.get(object.id)??[];for(const clip of list){if(time<clip.start)break;if(clip.kind==='transformFunction'){if(time>=clip.end)state.transform=clip.after;else state.transform=clip.provider(clip.easing((time-clip.start)/(clip.end-clip.start)));continue;}for(const [key,change] of Object.entries(clip.changes)){if(time>=clip.end)state[key]=change.after;else{const t=clip.easing((time-clip.start)/(clip.end-clip.start));if(key==='transform')state.transform=Transform2D.lerp(change.before,change.after,t);else if(key==='style')state.style=lerpStyleState(change.before,change.after,t);else state[key]=lerpNumber(change.before,change.after,t);}}if(time<clip.end)continue;}return state;}
  get frame(){return Frame.fromRenderer(this.renderer);}
  layout(...args){let options=args.at(-1);if(!options||typeof options!=='object'||!('to' in options))throw new TypeError('Scene.layout requires {to, duration?, easing?, at?}');args=args.slice(0,-1);let objects=args.length===1&&args[0] instanceof Group?args[0].children:args;const targets=options.to.targets(...objects),base=this.cursor;this.parallel(options.duration??1,api=>objects.forEach((o,i)=>api.animate(o,{transform:targets[i],duration:options.duration??1,easing:options.easing??Easing.SMOOTHSTEP,at:options.at??0})));this.cursor=Math.max(this.cursor,base+(options.at??0)+(options.duration??1));return objects;}
  seek(time){const t0=performance.now();this.time=Math.max(0,Math.min(this.duration||time,time));this.renderer.time=this.time;for(const o of this._trackedObjects.values()){assignState(o,this.stateAt(o,this.time));if(o instanceof CachedBatch2D&&this._batchInitial.has(o.id)&&this.time>=o.birth&&this.time<o.death)o.items=this.batchAt(o,this.time);}for(const v of this.values)v.value=this.valueAt(v,this.time);this.render();this.stats.seekMs=performance.now()-t0;return this;}
  render(){const t0=performance.now();this.renderer.resize();this.renderer.clear();this.renderer.time=this.time;if(this._renderListDirty){this._renderList=[...this.objects].sort((a,b)=>a.zIndex-b.zIndex);this._renderListDirty=false;}for(const o of this._renderList)if(o.visible&&this.time>=o.birth&&this.time<o.death)o.draw(this.renderer,this.camera.transform);this.stats.renderMs=performance.now()-t0;this.stats.frames++;}
  play({loop=false,from=0}={}){this.pause();this.playing=true;this._start=performance.now()-from*1000;const tick=now=>{if(!this.playing)return;let t=(now-this._start)/1000;if(this.duration&&t>this.duration){if(loop){this._start=now;t=0;}else{this.seek(this.duration);this.pause();return;}}this.seek(t);this._raf=requestAnimationFrame(tick);};this._raf=requestAnimationFrame(tick);return this;}
  pause(){this.playing=false;if(this._raf)cancelAnimationFrame(this._raf);this._raf=null;return this;}
  destroy(){this.pause();this._resizeObserver?.disconnect();this._resizeObserver=null;return this;}
  setMatrix(matrix){for(const o of this.objects)o.transform=Transform2D.fromMat2(matrix);for(const o of this.objects)this.initial.set(o.id,cloneState(o));this.render();}
  animateTo(target,duration=1000){const seconds=duration/1000,start=this.time||0,targets=this.objects.map(o=>[o,Transform2D.fromMat2(target)]);for(const [o,t] of targets)this.animate(o,{transform:t,duration:seconds,at:start});this.duration=Math.max(this.duration,start+seconds);this.play({from:start});}
}
