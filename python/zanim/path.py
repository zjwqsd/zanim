...
def _cubic_point(c: CubicBezierGeometry, t: float) -> Vec2:
    u = 1 - t
    return Vec2(
...
def geometry_points(geometry: Geometry, samples: int = 96) -> tuple[Vec2, ...]:
    ...
    if isinstance(geometry, CircleGeometry):
        return tuple(
            Vec2(
                geometry.radius * cos(2 * pi * i / samples),
                geometry.radius * sin(2 * pi * i / samples),
            )
    ...
    if isinstance(geometry, EllipseGeometry):
        return tuple(
            Vec2(
                geometry.radius_x * cos(2 * pi * i / samples),
                geometry.radius_y * sin(2 * pi * i / samples),
            )
            for i in range(samples + 1)
        )
...
def trim_geometry(geometry: Geometry, fraction: float) -> Geometry:
    ...
    if t <= 0.0:
        return LineGeometry(points[0], points[0])
    lengths = []
    total = 0.0
    ...
    if len(out) < 2:
        out.append(out[0])
    return PolylineGeometry(tuple(out))
...
def cubic_point(cubic: CubicBezierGeometry, t: float) -> Vec2:
...
def _split_cubic(cubic: CubicBezierGeometry) -> tuple[CubicBezierGeometry, CubicBezierGeometry]:
    ...
    return (
        CubicBezierGeometry(cubic.p0, p01, p012, mid),
        CubicBezierGeometry(mid, p123, p23, cubic.p3),
    )


def flatten_cubic(
    cubic: CubicBezierGeometry,
    *,
    ...
    def visit(curve: CubicBezierGeometry, depth: int) -> None:
        flatness = max(
